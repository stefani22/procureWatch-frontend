...
export default function HighRiskQueuePage({ onOpenContract }) {
  ...
  return (
    <div style={{ padding: "28px 32px", fontFamily: "'Segoe UI', sans-serif", maxWidth: 1100, margin: "0 auto" }}>
      ...
      <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", marginBottom: 24 }}>
        <div>
          ...
          <p style={{ margin: 0, color: "#666", fontSize: 14 }}>
            Contracts flagged as high risk requiring review
          </p>
        </div>
        {total > 0 && (
          <div style={{
            background: "#fff1f1", border: "1px solid #e74c3c", borderRadius: 8,
            padding: "8px 18px", color: "#c0392b", fontWeight: 700, fontSize: 15,
          }}>
      ...
      {error && (
        <div style={{
          background: "#fff1f1", border: "1px solid #e74c3c", color: "#c0392b",
          borderRadius: 8, padding: "12px 18px", marginBottom: 16, fontSize: 14,
        }}>
          ⚠️ {error}
        </div>
      )}
      ...
      <div style={{ background: "#fff", border: "1px solid #e8e8e8", borderRadius: 10, overflow: "hidden" }}>
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 14 }}>
          ...
          <tbody>
            {loading ? (
              <tr><td colSpan={7} style={{ padding: 40, textAlign: "center", color: "#aaa" }}>
              ...
              >
                <td style={{ padding: "12px 14px", fontWeight: 600, color: "#1a73e8" }}>
                  {c.noticeNumber || `#${c.id}`}
                </td>
                <td style={{ padding: "12px 14px", color: "#333", maxWidth: 200, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                  {c.supplier || "—"}
                </td>
                <td style={{ padding: "12px 14px", fontWeight: 500, color: "#222" }}>
                  {c.amount != null
                    ? new Intl.NumberFormat("mk-MK", { style: "currency", currency: "MKD", maximumFractionDigits: 0 }).format(c.amount)
                    : "—"}
                </td>
                <td style={{ padding: "12px 14px", color: "#666" }}>
                  {c.date && c.date !== "—"
                    ? new Date(c.date).toLocaleDateString("mk-MK")
                    : "—"}
                </td>
                <td style={{ padding: "12px 14px" }}>
                  <RiskBadge level={c.riskLevel || "UNKNOWN"} />
                </td>
                <td style={{ padding: "12px 14px", minWidth: 130 }}>
                  <ScoreBar score={c.riskScore} />
                </td>
                <td style={{ padding: "12px 14px" }}>
                  <button
                    onClick={e => { e.stopPropagation(); onOpenContract && onOpenContract(c.id); }}
                    style={{
